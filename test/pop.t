...
$resp = $pop->objects( attach => ['/test/monkey'] );
is($resp->isok, 1, "Attached using an array")
    or diag($resp->messages);
is_deeply( [$resp->value], [ '/test/monkey' ], "Got the list back" );
...
is_deeply(scalar($resp->value), $answer, "Setting TOD access" ) or diag( Dumper($resp) );
...
$pop->{exist} = 1;
$resp = $pop->objects( attach => '/test/monkey' );
is($resp->isok, 0, "Could not attach after a great EVIL");

$resp = $pop->objects( attach => ['/test/monkey'] );
is($resp->isok, 0, "Could not attach after a great EVIL, redux");

$resp = $pop->objects();
is($resp->isok, 0, "Could not find attach after a great EVIL");

$resp = $pop->anyothernw;
is($resp->isok,0, "Could not get the anyothernw policy after a great EVIL");

$resp = $pop->anyothernw('forbidden');
is($resp->isok,0, "Could not set the anyothernw policy after a great EVIL");

$resp = $pop->description;
is($resp->value,undef,"Proper return from description after a great EVIL");

$resp = $pop->description('forbidden');
is($resp->isok,0, "Could not set the description after a great EVIL") or diag(Dumper($resp));

$resp = $pop->audit;
is($resp->value,0,"Proper return from audit after a great EVIL");

$resp = $pop->audit('admin');
is($resp->isok,0, "Could not set the audit level after a great EVIL");

$resp = $pop->ipauth;
is($resp->isok,0, "Could not get an ipauth level after a great EVIL");

$resp = $pop->ipauth( add => { '192.168.11.0' => { NETMASK => '255.255.255.0',
						   AUTHLEVEL => 2 }});
is($resp->isok,0, "Could not set the ipauth level after a great EVIL");

$resp = $pop->qop('none');
is($resp->isok,0, "Could not set the qop level after a great EVIL");

$resp = $pop->qop();
is($resp->value,undef, "Could not get a qop level after a great EVIL");
...
is($resp->isok,0, "Could not set the tod after a great EVIL");
$resp = $pop->tod;
is_deeply(scalar($resp->value), {days => [qw/any/], start => '0000', end => '0000', reference => 'local'},
	 "Could not get a TOD after a great EVIL") or diag(Dumper($resp));

$resp = $pop->warnmode(1);
is($resp->isok,0, "Could not set the warnmode level after a great EVIL");

$resp = $pop->warnmode;
is($resp->value,0, "Could not get a warnmode after a great EVIL");

$resp = $pop->warnmode;
is($resp->value,0, "Could not get a name after a great EVIL");

$resp = $pop->attributes( add => { fooly => 'cooly' } );
is($resp->isok, 0, "Could not add attribute values after a great EVIL");

$resp = $pop->attributes( remove => { fooly => 'cooly' } );
is($resp->isok, 0, "Could not remove attribute values after a great EVIL");

$resp = $pop->attributes( add => { fooly => ['cooly'] } );
is($resp->isok, 0, "Could not add an array of attribute values after a great EVIL");

$resp = $pop->attributes( remove => { fooly => ['cooly'] } );
is($resp->isok, 0, "Could not remove an array of attribute values after a great EVIL");

$resp = $pop->attributes( removekey => [qw/fooly next/] );
is($resp->isok, 0, "Could not remove an array of attribute keys after a great EVIL");

$resp = $pop->attributes;
is($resp->isok, 0, "Could not get any attributes after a great EVIL");
